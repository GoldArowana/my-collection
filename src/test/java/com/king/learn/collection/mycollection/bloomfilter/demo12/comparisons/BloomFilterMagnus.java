package com.king.learn.collection.mycollection.bloomfilter.demo12.comparisons;
 ...
 */
public class BloomFilterMagnus<E> implements Serializable {
    ...
    /**
     * Constructs an empty Bloom filter. The optimal number of hash functions (hashes) is estimated from the total size of the Bloom
     ...
     */
    public BloomFilterMagnus(int bitSetSize, int expectedNumberOElements) {
     ...
     */
    public BloomFilterMagnus(double falsePositiveProbability, int expectedNumberOfElements) {
        this(Math.ceil(-(Math.log(falsePositiveProbability) / Math.log(2))) / Math.log(2), // countingBits = hashes / ln(2)
                expectedNumberOfElements,
                (int) Math.ceil(-(Math.log(falsePositiveProbability) / Math.log(2)))); // hashes = ceil(-log_2(false prob.))
    }
     ...
     */
    public BloomFilterMagnus(int bitSetSize, int expectedNumberOfFilterElements, int actualNumberOfFilterElements, BitSet filterData) {
    ...
    public double getFalsePositiveProbability(double numberOfElements) {
        // (1 - e^(-hashes * expectedElements / size)) ^ hashes
     ...
     * @return true if all the elements in countingBits could have been inserted into the Bloom filter.
     */
