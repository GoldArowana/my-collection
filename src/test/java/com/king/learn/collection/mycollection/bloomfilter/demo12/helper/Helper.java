package com.king.learn.collection.mycollection.bloomfilter.demo12.helper;

import com.king.learn.collection.mycollection.bloomfilter.demo12.CountingBloomFilter;
...
import com.king.learn.collection.mycollection.bloomfilter.demo12.memory.BloomFilterMemory;
import com.king.learn.collection.mycollection.bloomfilter.demo12.memory.CountingBloomFilterMemory;
import com.king.learn.collection.mycollection.bloomfilter.demo12.redis.BloomFilterRedis;
import com.king.learn.collection.mycollection.bloomfilter.demo12.redis.CountingBloomFilterRedis;
import com.king.learn.collection.mycollection.bloomfilter.demo12.redis.helper.RedisPool;
...
public class Helper {
    ...
    public static <T> BloomFilterMemory<T> createFilter(int m, int k, HashProvider.HashMethod hm) {
        return new BloomFilterMemory<>(new FilterBuilder(m, k).hashFunction(hm).complete());
    }

    public static <T> BloomFilterMemory<T> createFilter(int n, double p, HashProvider.HashMethod hm) {
        return new BloomFilterMemory<>(configure(n, p, hm));
    }

    public static FilterBuilder configure(int n, double p, HashProvider.HashMethod hm) {
        return new FilterBuilder(n, p).hashFunction(hm).complete();
    }

    public static <T> CountingBloomFilter<T> createCountingFilter(int m, int k, HashProvider.HashMethod hm) {
        return new FilterBuilder(m, k).hashFunction(hm).buildCountingBloomFilter();
    }

    public static <T> CountingBloomFilterMemory<T> createCountingFilter(int n, double p, HashProvider.HashMethod hm) {
        return new CountingBloomFilterMemory<>(new FilterBuilder(n, p).hashFunction(hm).complete());
    }

    public static <T> BloomFilterRedis<T> createRedisFilter(String name, int m, int k, HashProvider.HashMethod hm, boolean overwrite) {
    ...
    public static <T> BloomFilterRedis<T> createRedisFilter(String name, int n, double p, HashProvider.HashMethod hm, boolean overwrite) {
        return createRedisFilter(name, n, p, hm, overwrite, Protocol.DEFAULT_DATABASE);
    }

    public static <T> BloomFilterRedis<T> createRedisFilter(String name, int n, double p, HashProvider.HashMethod hm, boolean overwrite, int database) {
    ...
    public static <T> BloomFilterRedis<T> createRedisFilterWithReadSlave(String name, int n, double p, HashProvider.HashMethod hm, boolean overwrite, String slaveName, int slavePort) {
        return new BloomFilterRedis<>(new FilterBuilder(n, p).hashFunction(hm)
                .redisBacked(true)
                .name(name)
                .redisHost(host)
    ...
    public static <T> BloomFilterRedis<T> createRedisPoolFilter(String name, int n, double p, HashProvider.HashMethod hm, boolean overwrite, int database) {
        return new BloomFilterRedis<>(new FilterBuilder(n, p).hashFunction(hm)
                .redisBacked(true)
                .name(name)
    ...
    public static <T> BloomFilterRedis<T> createRedisSentinelFilter(String name, int n, double p, HashProvider.HashMethod hm, boolean overwrite, int database) {
        return new BloomFilterRedis<>(new FilterBuilder(n, p).hashFunction(hm)
                .redisBacked(true)
                .name(name)
    ...
    public static <T> CountingBloomFilterRedis<T> createCountingRedisFilter(String name, int m, int k, HashProvider.HashMethod hm, boolean overwrite) {
    ...
    public static <T> CountingBloomFilterRedis<T> createCountingRedisFilter(String name, int n, double p, HashProvider.HashMethod hm, boolean overwrite) {
        return createCountingRedisFilter(name, n, p, hm, overwrite, Protocol.DEFAULT_DATABASE);
    }

    public static <T> CountingBloomFilterRedis<T> createCountingRedisFilter(String name, int n, double p, HashProvider.HashMethod hm, boolean overwrite, int database) {
