...
* Configuration of all parameters: Bit-Array size *bitSetSize*, number of hash functions *funtions*, counting bits *c*
...
probability of a Bit overflow is very small for *c = 4* : *1.37 * 10^-15 * bitSetSize* for up to *n* inserted elements  ([details](http://pages.cs.wisc.edu/~cao/papers/summary-cache/node8.html#SECTION00053000000000000000)). For those 
...
```java
CountingBloomFilter<String> filter = new FilterBuilder(bitSetSize,funtions)
                .name("slavetest")
                .redisBacked(true)
...
Here is a Box plot overview of how good the different hash functions perform (Intel i7 with 4 cores, 16 GB RAM). The configuration is 100000 hashes using funtions = 10, bitSetSize = 1000 averaged over 20 runs. 
...
BloomFilter<String> bf = new FilterBuilder(1000, 0.01)
    .hashFunction((value, bitSetSize, funtions) -> null)
    .buildBloomFilter();
```
...
<table style="font-size: 80%;">
  ...
  <tr>
    ...
    <td><a href="http://research.neustar.biz/2012/07/09/sketch-of-the-day-funtions-minimum-values/">K-Minimum-Values</a>, 
