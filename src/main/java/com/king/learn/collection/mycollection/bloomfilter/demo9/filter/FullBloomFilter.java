package com.king.learn.collection.mycollection.bloomfilter.demo9.filter;
...
import com.king.learn.collection.mycollection.bloomfilter.demo9.hash.IntHashFunction;
...
public class FullBloomFilter<T> implements BloomFilter<T> {
     ...
     * @param size      Bloom filter size <b>in bits</b>. If this parameter is < 8,
     *                  then the filter will have the size of 8 bits. Otherwise the
     *                  size will be (size / 8) * 8 bits.
     * @param hf        An object extending the IntHashFunctionFactory
     ...
     *
     * @param expectedNoE       Expected number of elements in filter
     * @param falsePositiveProb Allowed false positive probability. Must be between 0 and 1.
     * @param hf                An object extending the IntHashFunctionFactory
     *                          (Murmur2Factory, for instance) which supplies the hash
     *                          functions.
     ...
     * bit in flags
     *
     * @param hashes Hash values array
     * @param index  Index of current hash in a loop
     */
