package com.king.learn.collection.mycollection.bloomfilter.demo5;
...
public class BloomFilter<E> implements Serializable {
    ...
    /**
     * Constructs an empty Bloom filter. The optimal number of hash functions (funtions) is estimated from the total size of the Bloom
    ...
    public BloomFilter(double falsePositiveProbability, int expectedNumberOfElements) {
        this(Math.ceil(-(Math.log(falsePositiveProbability) / Math.log(2))) / Math.log(2), // c = funtions / ln(2)
                expectedNumberOfElements,
                (int) Math.ceil(-(Math.log(falsePositiveProbability) / Math.log(2)))); // funtions = ceil(-log_2(false prob.))
    }
