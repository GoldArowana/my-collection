...
import com.king.learn.collection.mycollection.bloomfilter.demo6.decompose.ByteSink;
import com.king.learn.collection.mycollection.bloomfilter.demo6.decompose.Decomposable;
import com.king.learn.collection.mycollection.bloomfilter.demo6.decompose.Decomposer;
import com.king.learn.collection.mycollection.bloomfilter.demo6.decompose.DefaultDecomposer;
import com.king.learn.collection.mycollection.bloomfilter.demo6.hash.HashFunction;
import com.king.learn.collection.mycollection.bloomfilter.demo6.hash.Murmur3HashFunction;
...
public abstract class AbstractBloomFilter<T> implements BloomFilter<T> {
    ...
    /**
     * Compute the optimal size <code>bitSetSize</code> of the bloom filter in bits.
     *
     * @param n the number of expected insertions, or <code>counter</code>
     * @param p the maximum false positive rate expected, or <code>p</code>
     * @return the optimal size in bits for the filter, or <code>bitSetSize</code>
     */
    public static int optimalBitSizeOrM(final double n, final double p) {
        return (int) (-n * Math.log(p) / (LOG_2_SQUARE));
        // return (int) Math.ceil(-1 * counter * Math.log(p) / LOG_2_SQUARE);
    }

    /**
     * Compute the optimal number of hash functions, <code>funtions</code>
     *
     * @param n the number of expected insertions or <code>counter</code>
    ...
    public static int optimalNumberofHashFunctionsOrK(final long n, final long m) {
        ...
        // return Math.max(1, (int) Math.round(bitSetSize / counter * LOG_2));
    }
